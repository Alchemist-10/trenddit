...
with col2:
    source = st.multiselect("Source", options=["reddit","twitter"], default=["reddit","twitter"])
...
poll_interval = st.slider("Polling interval (seconds)", min_value=10, max_value=300, value=30)
...
if st.button("Save query"):
    ...
    if res.error:
        st.error(f"Failed to save query: {res.error.message if hasattr(res.error,'message') else res.error}")
    else:
...
def fetch_posts(keyword, sources, start_time, limit=1000, offset=0):
    ...
    if res.error:
        st.error(f"Error fetching posts: {res.error}")
        return []
    return res.data or []
...
with left:
    ...
    if df.empty:
        ...
        fig = px.line(timeline, x='created_at', y='avg_sentiment', title=f"Average sentiment for '{keyword}'")
        fig.add_bar(x=timeline['created_at'], y=timeline['volume'], name='volume', opacity=0.4, yaxis='y2')
        # add secondary axis
        fig.update_layout(
    ...
    if not df.empty:
        # combine title+body
        df_text = (df.get('title','') + " " + df.get('body','')).fillna('')
        # extract top n-grams
        ...
        freq_df = pd.DataFrame(sorted(freqs, key=lambda x: x[1], reverse=True), columns=['term','count'])
    ...
    if not df.empty and 'embedding' in df.columns:
        ...
        try:
            import ast
            emb_list = df['embedding'].apply(lambda x: np.array(x) if isinstance(x, list) else (np.array(ast.literal_eval(x)) if isinstance(x,str) else np.array([0]*384)))
            # simple KMeans
            from sklearn.decomposition import PCA
            from sklearn.cluster import KMeans
            E = np.vstack(emb_list.to_list())
            n_clusters = min(6, max(2, E.shape[0]//10))
            ...
            for c in sorted(df['cluster'].unique()):
                ...
                rep = subset.iloc[0]
                cluster_summary.append((c, len(subset), rep.get('title') or rep.get('body')[:200], rep.get('url')))
...
with right:
    ...
    page_size = st.number_input("Page size", min_value=5, max_value=100, value=10)
    # pagination controls
    page = st.number_input("Page", min_value=1, value=1)
    offset = (page-1)*page_size
    posts_page = fetch_posts(keyword, source, start_time, limit=page_size, offset=offset)
    if not posts_page:
        ...
        for p in posts_page:
            ...
            st.caption(f"{p.get('source')} • {p.get('author')} • {created} • sentiment: {score:.2f} ({label})")
            st.write(p.get('body') or '')

    # Alerts panel
    st.subheader("Alerts")
    alerts_res = supabase.table("alerts").select("*").order("triggered_at", desc=True).limit(10).execute()
    if alerts_res.error:
        ...
        if not alerts:
            ...
            for a in alerts:
                st.warning(f"[{a.get('alert_type')}] {a.get('message')} — {pretty_time_ago(a.get('triggered_at'))}")
...
if st.button("Export CSV"):
    if df.empty:
        ...
        st.download_button("Download posts CSV", data=csv, file_name=f"trenddit_{keyword}_{now.date()}.csv", mime="text/csv")
